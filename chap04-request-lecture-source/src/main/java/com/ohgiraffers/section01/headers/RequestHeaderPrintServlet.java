...
public class RequestHeaderPrintServlet extends HttpServlet {
    ...
    * */
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // accpet: 요청을 보낼 때 서버에게 요청할 응답 타입을 명시
        System.out.println("accept-encoding: " + request.getHeader("accept-encoding"));
        // accept-encoding: 응답시 원하는 인코딩 방식
        System.out.println("accept-language: " + request.getHeader("accept-language"));
        // accept-language: 응답시 원하는 언어
        System.out.println("connection: " + request.getHeader("connection"));
        // aonnection: HTTP 통신이 완료된 후에 네트워크 접속을 유지할지 결정 (기본값 keep-alive = 연결을 열린상태로 유지)
        System.out.println("host: " + request.getHeader("host"));
        // host: 서버의 도메인 이름과 서버가 현재 Listening 중인 TCP 포트 지정 (반드시 하나가 존재하고 두개면 404Error)
        ...
        // sec-fetch-site: 출처(origin)와 요청된 resourece 사이의 관계
        ...
        // upgrade-insecure-request: 클라이언트가 서버한테 http -> https로 업그레이드를 요청할때 사용
